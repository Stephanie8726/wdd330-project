...
function createTable(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const statusRow = createElement(
      "div",
      { textContent: data[i].progress },
      []
    );

    const editButton = createElement(
      "button",
      { className: "editButton", textContent: "Edit" },
      []
    );
    const editRow = createElement("div", {}, [editButton]);
    editRow.addEventListener("click", async () => {
      window.location.hash = `#/tracking?goalId=${
        data[i].id
      }&goalName=${encodeURIComponent(data[i].name)}`;
    });
...
function Dashboard() {
  ...
  fetch("https://664d5b4fede9a2b556534a27.mockapi.io/api/fit/activities")
    ...
    .then((activities) => {
      ...
      const bodyTable = createTable(activities.filter(a => a.userId === userId));
